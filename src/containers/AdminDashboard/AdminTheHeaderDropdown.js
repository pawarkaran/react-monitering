...
const TheHeaderDropdown = ({ setAuth }) => {
  ...
  const [Adminfirstname, setAdminName] = useState("");
  ...
  const getAdmin = async () => {
    try {
      ...
      setAdminName(parseData.user_firstname);
      setAdminLastname(parseData.user_lastname);
    } catch (err) {
    ...
    >
      <CDropdownToggle className="c-header-nav-link" caret={false}>
        <div className="c-avatar">
          ...
          <p className="mfe-4 text-center pt-1 ">{Adminfirstname} {Adminlastname}</p>
        </div>
      </CDropdownToggle>
